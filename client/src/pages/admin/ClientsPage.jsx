...
const ClientsPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Basic Validation: Ensure no empty spaces are submitted
    ...
    <div className="p-4 max-w-7xl mx-auto">
      ...
      {isAdding && (
        <div className="bg-white rounded-xl shadow-lg p-6 mb-10 border border-gray-100">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-lg p-4">
              {formData.image ? (
                ...
                <div className="text-gray-400 text-sm">Enter a URL to see preview</div>
              )}
            </div>
          </div>
          ...
          >
            {editingId ? 'Update Client Info' : 'Save Client'}
          </button>
        </div>
      )}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {clients.length > 0 ? (
          clients.map((client) => (
            <div key={client.id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-shadow p-6 border border-gray-50">
              <div className="text-center">
                ...
                />
                <h3 className="text-xl font-bold text-gray-800 leading-tight">{client.name}</h3>
                <p className="text-sm font-medium text-blue-600 uppercase tracking-wide mb-3">{client.designation}</p>
                <p className="text-gray-600 italic text-sm mb-6">"{client.description}"</p>
                ...
                <div className="flex gap-3 border-t pt-4">
                  ...
                  <button
                    onClick={() => {
                      if (window.confirm(`Are you sure you want to remove ${client.name}?`)) deleteClient(client.id);
                    }}
          ...
          <div className="col-span-full text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed">
            <p className="text-gray-500 text-lg">No clients found. Click "+ Add Client" to get started!</p>
          </div>
        )}
      </div>
    </div>
